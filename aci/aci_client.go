...
func (c *Client) configProxy(transport *http.Transport) *http.Transport {
	log.Debugf("Using Proxy Server: %s ", c.proxyUrl)
...
func (c *Client) MakeRestRequestRaw(method string, rpath string, payload []byte, authenticated bool) (*http.Request, error) {
	...
	log.Debugf("BaseURL: %s, pathURL: %s, fURL: %s", c.BaseURL.String(), pathURL.String(), fURL.String())
	if method == "GET" {
	...
	if c.skipLoggingPayload {
		log.Debugf("HTTP request %s %s", method, rpath)
	} else {
		log.Debugf("HTTP request %s %s %v", method, rpath, req)
	}
	if authenticated {
	...
	if !c.skipLoggingPayload {
		log.Debugf("HTTP request after injection %s %s %v", method, rpath, req)
	}
...
func (c *Client) Authenticate() error {
	...
	if err != nil {
		log.Debugf("Authentication ERROR: %s", err)
		return err
	}
	if obj == nil {
...
func (c *Client) Do(req *http.Request) (*container.Container, *http.Response, error) {
	log.Debugf("Begining Do method %s", req.URL.String())
	if len(c.cookies) > 0 {
	...
	for attempts := 0; ; attempts++ {
		log.Tracef("HTTP Request Method and URL: %s %s", req.Method, req.URL.String())
		if c.maxRetries != 0 {
			req.Body = ioutil.NopCloser(bytes.NewBuffer(body))
		}
		if !c.skipLoggingPayload {
			log.Tracef("HTTP Request Body: %v", req.Body)
		}

		resp, err := c.httpClient.Do(req)
		if err != nil {
			if ok := c.backoff(attempts); !ok {
				log.Errorf("HTTP Connection error occured: %+v", err)
				log.Debugf("Exit from Do method")
				return nil, nil, fmt.Errorf("failed to connect to APIC. Verify that you are connecting to an APIC.\nError message: %+v", err)
			} else {
				log.Errorf("[ERROR] HTTP Connection failed: %s, retries: %v", err, attempts)
				continue
			}
		}

		if !c.skipLoggingPayload {
			log.Tracef("HTTP Response: %d %s %v", resp.StatusCode, resp.Status, resp)
		} else {
			log.Tracef("HTTP Response: %d %s", resp.StatusCode, resp.Status)
		}

		bodyBytes, err := ioutil.ReadAll(resp.Body)
		if err != nil {
			log.Errorf("An error occurred reading response: %s", err)
		}
		bodyStr := string(bodyBytes)
		resp.Body.Close()
		if !c.skipLoggingPayload {
			log.Debugf("HTTP response unique string %s %s %s", req.Method, req.URL.String(), bodyStr)
		}

		if (resp.StatusCode < 500 || resp.StatusCode > 504) && resp.StatusCode != 405 {
			obj, err := container.ParseJSON(bodyBytes)
			if err != nil {
				log.Errorf("Error occured while json parsing: %+v", err)
				...
				log.Errorf("Error occured while json parsing: %s", htmlErr.Error())
				log.Debug("Exit from Do method")
			...
			if ok := c.backoff(attempts); !ok {
				obj, err := container.ParseJSON(bodyBytes)
				if err != nil {
					log.Errorf("Error occured while json parsing: %+v with HTTP StatusCode 405, 500-504", err)
				...
				log.Errorf("HTTP Request failed: StatusCode %v, Retries: %v", resp.StatusCode, attempts)
				continue
			}
		}
	}
...
func (c *Client) DoRaw(req *http.Request, result interface{}) (*http.Response, error) {
	log.Debugf("Begining DoRaw method %s", req.URL.String())
	if len(c.cookies) > 0 {
		for _, cookie := range c.cookies {
			req.AddCookie(cookie)
		}
	...
	for attempts := 0; ; attempts++ {
		...
		if err != nil {
			if ok := c.backoff(attempts); !ok {
				log.Errorf("HTTP Connection error occured: %+v", err)
				log.Debugf("Exit from DoRaw method")
				return nil, fmt.Errorf("failed to connect to APIC. Verify that you are connecting to an APIC.\nError message: %+v", err)
			} else {
...
func (c *Client) backoff(attempts int) bool {
	log.Debugf("Beginning backoff method: attempts %v on %v", attempts, c.maxRetries)
	if attempts >= c.maxRetries {
		log.Debug("Exit from backoff method with return value false")
		return false
	}
	...
	log.Trace("Starting sleeping for %v", backoffDuration.Round(time.Second))
	time.Sleep(backoffDuration)
	log.Debug("Exit from backoff method with return value true")
	return true
}
...
func (c *Client) checkHtmlResp(body string) error {
	...
	log.Debugf("HTML Error Parsing Result: %s", errStr)
